...
router.get('/passsrecovery', async (req, res, next)=>{
  try {
    mailSender(
      "jm.hernandezrivera@gmail.com",
      "¿Hola Jose como estas?",
      '<h1>Esto es un prueba de correo</h1><p>Su correo es: '+req.user.mail+'</p><p>Click aqui para setear contraseña <a href="http://localhost:3000/recovery">CLICK ME</></p>'
    );
  } catch (ex) {
    console.log(ex);
    res.status(500).json({ "msg": "Error" });
